...
async def execute_tool(name: str, args: list) -> str:
    if name == "calculate":
    ...
    if name == "open_app":
        return await _run_skill("open_app", app=args[0]) if args else "Error: missing app name"
    if name == "set_volume":
        return await _run_skill("set_volume", level=int(args[0])) if args else "Error: missing level"
    if name == "system_control":
        return await _run_skill("system_control", action=args[0]) if args else "Error: missing action"
    if name == "set_brightness":
        return await _run_skill("set_brightness", level=int(args[0])) if args else "Error: missing level"
...
class ReactAgent(BaseAgent):
    ...
    async def _llm(self, messages: list[ChatMessage]) -> str:
        """
        Appelle le moteur LLM via la couche engine et renvoie le contenu texte
        du dernier message assistant.
        """
        engine = get_engine(self.engine_id)
        result: ChatResult = await engine.chat(messages=messages)
    ...
    async def run(self, query: str, context: AgentContext = None) -> str:
        ...
        for pattern, builder in DIRECT_PATTERNS:
            ...
            if m:
                ...
                return await _run_skill(skill_name, **kwargs)
        ...
        ctx.add_user(
            f"Before solving, briefly plan your approach in 2-3 steps:\nQuestion: {query}"
        )
        plan = await self._llm(ctx.messages)  # type: ignore[arg-type]
        ...
        for _ in range(8):
            response = await self._llm(ctx.messages)  # type: ignore[arg-type]
            full_trace += response + "\n"
    ...
    async def _verify(self, query: str, answer: str, trace: str) -> str:
        verification_prompt: list[ChatMessage] = [
            {
                "role": "system",
                "content": (
                    "You are a careful reviewer. Check if the answer is correct, "
            ...
            },
            {
                ...
                "content": (
                    ...
                    "Is this answer correct and complete? If yes, return it as-is. "
                    "If not, correct it. Return only the final answer, nothing else."
                ),
            },
        ]
        return await self._llm(verification_prompt)